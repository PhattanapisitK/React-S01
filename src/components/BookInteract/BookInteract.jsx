...
function BookInteract({ book, mode }) {
    ...
    const update = async (id) => {
        await axios.put(`https://node57538-phattanapisitk.proen.app.ruk-com.cloud/books/${id}`, payload)
            .then((resp) => {
                if (resp.status == 200) {
    ...
    const add = async () => {
        await axios.post('https://node57538-phattanapisitk.proen.app.ruk-com.cloud/books', payload)
            .then((resp) => {
                if (resp.status == 200) {
                    window.location.reload();
